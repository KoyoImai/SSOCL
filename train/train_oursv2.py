...
def train_oursv2(model, model2, criterions, optimizer, trainloader, cfg, epoch, ckpt_manager=None, writer=None, scaler=None):
    ...
    distill_losses = AverageMeter('DistillLoss', ':.4e')
    ...
    if cfg.continual.buffer_type in ["minred"]:
        ...
        neig_similarity]
    progress = ProgressMeter(len(trainloader), [batch_time, data_time, lr_meter] + buff_meters + [losses] + [mcc_losses] + [tcr_losses] + [distill_losses],
    ...
    criterion_distill = criterions["distill"]
    ...
    for idx, data in enumerate(trainloader):
        ...
        encoded, feature, z_proj, ema_encoded, ema_feature, ema_z_proj = model(images)
        z_proj_concat = concat_all_gather_keep_grad(z_proj)  # [B_global * V, D]
        ema_z_proj_concat = concat_all_gather_keep_grad(ema_z_proj)  # [B_global * V, D]
        
        z_list = z_proj_concat.chunk(cfg.method.num_crops, dim=0)
        z_avg = chunk_avg(z_proj, cfg.method.num_crops)
        ema_z_list = ema_z_proj_concat.chunk(cfg.method.num_crops, dim=0)
        # ema_z_avg = chunk_avg(ema_z_proj, cfg.method.num_crops)
        # print("ema_z_avg.shape: ", ema_z_avg.shape)    # [batch_size, dim]


        loss_mcc = criterion_mcc(z_list)
        loss_distill = criterion_distill(z_list, ema_z_list)
        loss_tcr = cal_TCR(z_proj, criterion_tcr, cfg.method.num_crops)
        mcc_losses.update(loss_mcc.item(), images.size(0))
        tcr_losses.update(loss_tcr.item(), images.size(0))
        distill_losses.update(loss_distill.item(), images.size(0))
    
        loss = cfg.method.lambda_mcc * loss_mcc + cfg.method.lambda_tcr * loss_tcr + cfg.method.lambda_distill * loss_distill
        losses.update(loss.item(), images.size(0))
