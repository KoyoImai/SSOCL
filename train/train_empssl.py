...
def train_empssl(model, model2, criterions, optimizer, trainloader, cfg, epoch, ckpt_manager=None, writer=None, scaler=None):


    # 学習状況を記録するための meter
    batch_time = WindowAverageMeter('Time', fmt=':6.3f')
    data_time = WindowAverageMeter('Data', fmt=':6.3f')
    sim_losses = AverageMeter('SimLoss', ':.4e')
    ...
    if cfg.continual.buffer_type in ["minred", "random"]:
        ...
        neig_similarity]
    progress = ProgressMeter(len(trainloader), [batch_time, data_time, lr_meter] + buff_meters + [losses] + [sim_losses] + [tcr_losses],
    ...
    for idx, data in enumerate(trainloader):
        ...
        if use_amp:
            with autocast(dtype=amp_dtype):
                ...
                loss_sim, _ = criterion_sim(z_list, z_avg)
                loss_tcr = cal_TCR(z_proj, criterion_tcr, cfg.method.num_crops)
                sim_losses.update(loss_sim.item(), images.size(0))
                tcr_losses.update(loss_tcr.item(), images.size(0))

                loss = cfg.method.lambda_sim * loss_sim + cfg.method.lambda_tcr * loss_tcr
