...
class MixMinRedBufferBatchSampler(BaseBufferBatchSampler):
    ...
    def sample_k_mem_oonly(self, buffer, mem_batch_size):
        
        if mem_batch_size < len(buffer):
            return random.sample(buffer[:- self.batch_size], k=mem_batch_size)
        
        assert False
    
    
    def make_batch_mem_only(self):

        # バッファから mem_batch_size 分のデータをランダムで取り出す
        # （データストリームからのバッチと重複しないように取り出す）
        mem_batch = self.sample_k_mem_oonly(self.buffer, self.mem_batch_size)

        # データセットの idx を取り出す
        mem_batch_idx = [b['idx'] for b in mem_batch]

        return mem_batch_idx
    ...
    def __iter__(self):
        ...
        while self.num_batches_yielded < len(self):
            ...
            # self.bufferにデータを追加
            _, indices_to_add = self.add_to_buffer(self.batch_size)
            ...
            for j in range(self.repeat):
                ...
                # リプレイバッファから取り出したデータでミニバッチを作成
                mem_batch_idx = self.make_batch_mem_only()
                # print("mem_batch_idx: ", mem_batch_idx)
                # print("indices_to_add: ", indices_to_add)
                # print("len(mem_batch_idx): ", len(mem_batch_idx))

                # リプレイバッファとデータストリームからミニバッチを作成
                batch_idx = self.indices_to_add + mem_batch_idx
